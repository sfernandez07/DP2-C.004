...
public class AirlineManagerFlightLegPublishService extends AbstractGuiService<AirlineManager, FlightLeg> {
	...
	public void authorise() {
		...
		boolean isAuthorized = flightLeg != null && flightLeg.getFlight().getManager().equals(loggedInManager) && flightLeg.getFlight().getStatus() == Status.NOT_READY && flightLeg.getStatus() == Status.NOT_READY;
	...
	public void validate(final FlightLeg flightLeg) {
		boolean isValidStatus = flightLeg.getLegStatus() == LegStatus.ON_TIME || flightLeg.getLegStatus() == LegStatus.DELAYED;
	...
	public void perform(final FlightLeg flightLeg) {
		flightLeg.setStatus(Status.READY);
