		...
		(
				...
				(function(exports, $, Popper) {
						...
						function getSpecialTransitionEndEvent() {
							return {
								...
								handle : function handle(event) {
									if ($$$1(event.target).is(this)) { return event.handleObj.handler.apply(this, arguments); // eslint-disable-line prefer-rest-params
									}
							...
							typeCheckConfig : function typeCheckConfig(componentName, config, configTypes) {
								for ( var property in configTypes) {
									if (Object.prototype.hasOwnProperty.call(configTypes, property)) {
										...
										if (!new RegExp(expectedTypes).test(valueType)) { throw new Error(componentName.toUpperCase() + ": "
								...
								if (rootElement) {
									...
									if (input) {
										...
										if (triggerChangeEvent) {
											if (input.hasAttribute('disabled') || rootElement.hasAttribute('disabled') || input.classList.contains('disabled')
													|| rootElement.classList.contains('disabled')) { return; }
					...
					var Collapse = function($$$1) {
						...
						function() {
							function Collapse(element, config) {
								...
								this._element = element;
								this._config = this._getConfig(config);
								this._triggerArray = $$$1.makeArray(document.querySelectorAll("[data-toggle=\"collapse\"][href=\"#" + element.id + "\"],"
					...
					var Dropdown = function($$$1) {
						...
						function() {
							...
							Dropdown._dataApiKeydownHandler = function _dataApiKeydownHandler(event) {
								...
								if (/input|textarea/i.test(event.target.tagName) ? event.which === SPACE_KEYCODE || event.which !== ESCAPE_KEYCODE
										&& (event.which !== ARROW_DOWN_KEYCODE && event.which !== ARROW_UP_KEYCODE || $$$1(event.target).closest(Selector.MENU).length)
						...
						$$$1(document).on(Event.KEYDOWN_DATA_API, Selector.DATA_TOGGLE, Dropdown._dataApiKeydownHandler).on(Event.KEYDOWN_DATA_API, Selector.MENU,
								Dropdown._dataApiKeydownHandler).on(Event.CLICK_DATA_API + " " + Event.KEYUP_DATA_API, Dropdown._clearMenus).on(Event.CLICK_DATA_API,
					...
					var Popover = function($$$1) {
						...
						var Default = _objectSpread({}, Tooltip.Default, {
							...
							trigger : 'click',
							content : '',
							template : '<div class="popover" role="tooltip">' + '<div class="arrow"></div>' + '<h3 class="popover-header"></h3>'
					...
					var ScrollSpy = function($$$1) {
						...
						function() {
							function ScrollSpy(element, config) {
								...
								this._config = this._getConfig(config);
								this._selector = this._config.target + " " + Selector.NAV_LINKS + "," + (this._config.target + " " + Selector.LIST_ITEMS + ",")
								...
								targets.map(function(element) {
									...
									if (target) {
										...
										if (targetBCR.width || targetBCR.height) {
											...
											return [
													$$$1(target)[offsetMethod]().top + offsetBase, targetSelector
											];
										}
									}
							...
							_proto._activate = function _activate(target) {
								...
								if ($link.hasClass(ClassName.DROPDOWN_ITEM)) {
									...
									$link.parents(Selector.NAV_LIST_GROUP).prev(Selector.NAV_LINKS + ", " + Selector.LIST_ITEMS).addClass(ClassName.ACTIVE); // Handle special
																																								// case when
																																								// .nav-link is
																																								// inside .nav-item
					...
					var Tab = function($$$1) {
						...
						function() {
							...
							_proto.show = function show() {
								...
								if (this._element.parentNode && this._element.parentNode.nodeType === Node.ELEMENT_NODE && $$$1(this._element).hasClass(ClassName.ACTIVE)
										|| $$$1(this._element).hasClass(ClassName.DISABLED)) { return; }
					...
					(function($$$1) {
						...
						if (version[0] < ltMajor && version[1] < minMinor || version[0] === minMajor && version[1] === minMinor && version[2] < minPatch || version[0] >= maxMajor) { throw new Error(
